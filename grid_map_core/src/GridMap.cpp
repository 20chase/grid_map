...
GridMap GridMap::getTransformedSubMap(const Eigen::Isometry3d& transform, 
                                      const Length& length, 
  ...
  newMap.setBasicLayers(basicLayers_);
  newMap.setTimestamp(timestamp_);
  newMap.setFrameId(frameId);
  newMap.setGeometry(length, resolution_, Position(0.0, 0.0));
  newMap.startIndex_.setZero();

  // Initialization.
  std::vector<Position> positionSamples;
  Position center;
  Index newIndex;

  const double sampleLength = resolution_ * sampleRatio;

  for (GridMapIterator iterator(newMap); !iterator.isPastEnd(); ++iterator) {
    // Get position at current index.
    if (!newMap.getPosition(*iterator, center)) {
      continue;
    }

    Position3 center3d(center.x(), center.y(), 0.0);
    Position3 center3dInOldGrid = transform * center3d;
    ...
    if (sampleRatio > 0.0) {
      positionSamples.reserve(5);
      positionSamples.push_back(Position(center3dInOldGrid.x(), center3dInOldGrid.y()));
      positionSamples.push_back(Position(center3dInOldGrid.x() - sampleLength, center3dInOldGrid.y()));
      positionSamples.push_back(Position(center3dInOldGrid.x() + sampleLength, center3dInOldGrid.y()));
      positionSamples.push_back(Position(center3dInOldGrid.x(), center3dInOldGrid.y() - sampleLength));
      positionSamples.push_back(Position(center3dInOldGrid.x(), center3dInOldGrid.y() + sampleLength));
    } else {
      positionSamples.push_back(Position(center3dInOldGrid.x(), center3dInOldGrid.y()));
    }
    ...
    // Copy the layers.
    for (const auto& layer : layers_) {
      auto& currentValueInNewGrid = newMap.at(layer, *iterator);
      float oldGridValue = 0.0;
      float numValid = 0.0;
      for (const auto& position : positionSamples) {
        // Get new index.
        if (!getIndex(position, newIndex)) {
          continue;
        }
        oldGridValue += at(layer, newIndex);
        numValid += 1.0;
      }
      if ( numValid != 0.0 ) {
        currentValueInNewGrid = oldGridValue / numValid;
      }
    }
  }
